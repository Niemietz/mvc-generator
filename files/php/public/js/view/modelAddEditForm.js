...
const ${page.item.name.capitaliseFirstLetter()}IdAttribute = "${page.item.name.replaceAt(0, page.item.name.charAt(0).toLowerCase())}Id";
...
document.addEventListener(contentLoadedEventListener, (event) => {
    document.getElementById(btnEditId).onclick = function(evt) {
        const ${page.item.name.replaceAt(0, page.item.name.charAt(0).toLowerCase())}Id = this.getAttribute(${page.item.name.capitaliseFirstLetter()}IdAttribute);
