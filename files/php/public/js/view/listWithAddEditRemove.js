exports.getListWithAddEditRemoveJSText = function(page) {
    let result =
`import { add${page.item.name.capitaliseFirstLetter()}, edit${page.item.name.capitaliseFirstLetter()}, delete${page.item.name.capitaliseFirstLetter()}, get${page.item.name.capitaliseFirstLetter()}s } from './api.js';

const lab${page.item.name.capitaliseFirstLetter()}TitleId = "add-edit-${page.item.name.replaceAt(0, item.page.item.name.charAt(0).toLowerCase())}-title"
const mdl${page.item.name.capitaliseFirstLetter()}Id = "mdl-add-edit-${page.item.name.replaceAt(0, item.page.item.name.charAt(0).toLowerCase())}"
...
`
    ...
    document.getElementById(btnAddEditId).onclick = function(evt) {
        const ${page.item.name.replaceAt(0, page.item.name.charAt(0).toLowerCase())}Id = -1
        if (this.getAttribute(${page.item.name.replaceAt(0, page.item.name.charAt(0).toLowerCase())}IdAttribute) != null) {
